...
from .conmamba import ConmambaEncoder, MambaDecoder
...
class TransformerInterface(nn.Module):
    ...
    def __init__(
        ...
        branchformer_activation=nn.GELU,
        attention_type: str | None = 'regularMHA',
        max_length: int | None = 2500,
        causal: bool | None = False,
        encoder_kdim: int | None = None,
        encoder_vdim: int | None = None,
        decoder_kdim: int | None = None,
        decoder_vdim: int | None = None,
        mamba_config=None,
    ):
        ...
        self.encoder: TransformerEncoder | ConformerEncoder | ConmambaEncoder | None = None
        if num_encoder_layers > 0:
            ...
            if encoder_module == 'transformer':
                ...
                assert normalize_before, 'normalize_before must be True for Conmamba'

                assert conformer_activation is not None, 'conformer_activation must not be None'

        # initialize the decoder
        self.decoder: TransformerDecoder | MambaDecoder | None = None
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, input_size, max_len=2500):
        ...
        if input_size % 2 != 0:
            raise ValueError(
                f'Cannot use sin/cos positional '
                f'encoding with odd channels (got channels={input_size})'
            )
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        ffn_type='regularFFN',
        ffn_cnn_kernel_size_list=None,
        causal=False,
    ):
        super().__init__()

        if ffn_cnn_kernel_size_list is None:
...
class TransformerEncoder(nn.Module):
    ...
    def forward(
        ...
        """
        assert dynchunktrain_config is None, 'Dynamic Chunk Training unsupported for this encoder'

        output = src
        keep_probs = self.rng.random(len(self.layers)) if self.layerdrop_prob > 0.0 else None
        attention_lst = []
        for i, enc_layer in enumerate(self.layers):
            if (
                not self.training
                or self.layerdrop_prob == 0.0
                or keep_probs[i] > self.layerdrop_prob  # type: ignore
            ):
