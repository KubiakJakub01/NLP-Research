...
def make_transformer_src_mask(
    src: torch.Tensor,
    causal: bool = False,
    dynchunktrain_config: DynChunkTrainConfig | None = None,
) -> torch.Tensor | None:
...
class TransformerASR(TransformerInterface):
    ...
    positional_encoding: str, optional
        ...
        e.g. 'fixed_abs_sine' for fixed absolute positional encodings.
    ...
    conformer_activation: torch.nn.Module, optional
        Activation module used after Conformer convolutional layers.
        E.g. Swish, ReLU etc. it has to be a torch Module.
    branchformer_activation: torch.nn.Module, optional
        Activation module used within the Branchformer Encoder.
        E.g. Swish, ReLU etc. it has to be a torch Module.
    ...
    def __init__(
        ...
        kernel_size: int | None = 31,
        bias: bool | None = True,
        encoder_module: str | None = 'transformer',
        decoder_module: str | None = 'transformer',
        conformer_activation: nn.Module | None = Swish,
        branchformer_activation: nn.Module | None = nn.GELU,  # type: ignore
        attention_type: str | None = 'regularMHA',
    ...
    def forward(self, src, tgt, wav_len=None, pad_idx=0):
        ...
        wav_len: torch.Tensor, optional
            ...
            containing the relative length to padded length for each example.
        ...
        (
            ...
            src_mask,
            tgt_mask,
        ) = make_transformer_src_tgt_masks(src, tgt, wav_len, causal=self.causal, pad_idx=pad_idx)
    ...
    def encode(
        ...
        encoder_out, _ = self.encoder(  # type: ignore
            src=src,
            src_mask=src_mask,
            src_key_padding_mask=src_key_padding_mask,
    ...
    def encode_streaming(self, src, context: TransformerASRStreamingContext):
        ...
        encoder_out, _ = self.encoder.forward_streaming(  # type: ignore
            src=src, pos_embs=pos_embs_source, context=context.encoder_context
        )
        return encoder_out

    def make_streaming_context(
        self, dynchunktrain_config: DynChunkTrainConfig, encoder_kwargs=None
    ):
        ...
        return TransformerASRStreamingContext(
            dynchunktrain_config=dynchunktrain_config,
            encoder_context=self.encoder.make_streaming_context(  # type: ignore
...
class EncoderWrapper(nn.Module):
    ...
    **kwargs : dict
    ...
    def make_streaming_context(self, *args, **kwargs):
        """Initializes a streaming context. Forwards all arguments to the underlying transformer.
        See :meth:`speechbrain.lobes.models.transformer.TransformerASR.make_streaming_context`.
        """
