...
class Parser:
    ...
    def is_done(self, prefixes: tuple[str, ...] | None = None) -> bool:
        return self.index >= len(self.lines) or (  # type: ignore
            prefixes and len(prefixes) > 0 and self._norm(self._cur_line()).startswith(prefixes)
        )
...
def text_to_patch(text: str, orig: dict[str, str]) -> tuple[Patch, int]:
    ...
    if (
        len(lines) < 2
        or not Parser._norm(lines[0]).startswith('*** Begin Patch')  # pylint: disable=protected-access
        or Parser._norm(lines[-1]) != '*** End Patch'  # pylint: disable=protected-access
    ):
