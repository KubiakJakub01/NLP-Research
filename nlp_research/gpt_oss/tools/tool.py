...
class Tool(ABC):
    ...
    def output_channel_should_match_input_channel(self) -> bool:
        """
        A flag which indicates whether the output channel
        of the tool should match the input channel.
        """
        return True
    ...
    async def process(self, message: Message) -> AsyncIterator[Message]:
        ...
        If implementing a tool that has to block while calling
        a function use `call_on_background_thread` to get a coroutine.
    ...
    def error_message(
        self, error_message: str, id_: UUID | None = None, channel: str | None = None
    ) -> Message:
        """
        Return an error message that's from this tool.
        """
        return Message(
            id=id_ if id_ else uuid4(),
