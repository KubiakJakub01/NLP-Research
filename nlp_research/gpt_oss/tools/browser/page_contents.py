...
def multiple_replace(text: str, replacements: dict[str, str]) -> str:
    ...
    regex = re.compile(
        rf'({"|".join(map(re.escape, replacements.keys()))})',
    )
...
def _clean_links(root: lxml.html.HtmlElement, cur_url: str) -> dict[str, str]:
    """Processes all anchor tags in the HTML,
    replaces them with a custom format and returns an ID-to-URL mapping.
    """
    ...
    for a in root.findall('.//a[@href]'):
        ...
        try:
            ...
            logger.debug('SKIPPING LINK WITH URL %s: %s', link, e)
            domain = ''
        if not domain:
...
def _escape_md_section(text: str) -> str:
    return text
...
def replace_images(
    ...
    (includes alt/title if available).
    ...
    for i, img_tag in enumerate(root.findall('.//img')):
        ...
        replacement = f'[Image {i}: {image_name}]' if image_name else f'[Image {i}]'
...
def process_html(
    ...
    _remove_math(root)
    clean_html = lxml.etree.tostring(root, encoding='UTF-8').decode()  # pylint: disable=no-member
    text = html_to_text(clean_html)
    text = re.sub(WHITESPACE_ANCHOR_RE, lambda match: match.group(2) + match.group(1), text)
