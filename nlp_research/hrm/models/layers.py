...
class CastedLinear(nn.Module):
    ...
    def forward(self, input_: torch.Tensor) -> torch.Tensor:
        return F.linear(  # pylint: disable=not-callable
            input_,
            self.weight.to(input_.dtype),
            bias=self.bias.to(input_.dtype) if self.bias is not None else None,
        )
...
class CastedEmbedding(nn.Module):
    ...
    def forward(self, input_: torch.Tensor) -> torch.Tensor:
        return F.embedding(input_, self.embedding_weight.to(self.cast_to))
