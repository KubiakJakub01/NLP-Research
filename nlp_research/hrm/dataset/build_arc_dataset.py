...
def puzzle_hash(puzzle: dict):
    ...
    for _, example in puzzle.items():
        for input_, label in example.examples:
            hashes.append(f'{_grid_hash(input_)}|{_grid_hash(label)}')
        ...
        for _trial in range(ARCAugmentRetriesFactor * aug_count):
            ...
            augmented = {
                dest: ARCPuzzle(
                    ...
                    [
                        (
                            dihedral_transform(mapping[input], trans_id),
                            dihedral_transform(mapping[label], trans_id),
                        )
...
def load_puzzles_arcagi(
    results: dict[str, dict[str, list[ARCPuzzle]]], dataset_path: str, config: DataProcessConfig
):
    ...
    for subdir in os.scandir(dataset_path):
        if subdir.is_dir():
            ...
            for idx, (default_name, puzzle) in enumerate(puzzles):
                ...
                for value, dest in test_examples_map.get(
            ...
            for group in subset:
                for puzzle in group:
                    ...
                    for _idx_ex, (inp, out) in enumerate(puzzle.examples):
                        result = np_grid_to_seq_translational_augment(
                            inp,
                            out,
                            do_translation=enable_translational_augment and _idx_ex != no_aug_id,
                        )

                        results['inputs'].append(result[0])
                        results['labels'].append(result[1])
                        example_id += 1
