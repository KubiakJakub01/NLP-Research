...
class GPTHparams(HParams):
    ...
    # Model
    n_embd: int = field(default=768, metadata={'help': 'Size of embedding layer.'})
    n_layer: int = field(default=12, metadata={'help': 'Number of transformer layers.'})
    n_head: int = field(default=12, metadata={'help': 'Number of attention heads.'})
    latent_size: int = field(default=0, metadata={'help': 'Size of latent vector.'})
    n_ctx: int = field(default=1024, metadata={'help': 'Size of context window.'})
    vocab_size: int = field(default=50257, metadata={'help': 'Size of vocabulary.'})
    n_positions: int = field(default=1024, metadata={'help': 'Number of positional embeddings.'})
    ...
    learning_rate: float = field(default=6.25e-5, metadata={'help': 'Learning rate for training.'})
    weight_decay: float = field(default=0.01, metadata={'help': 'Weight decay for training.'})
...
class UNetHparams(HParams):
    ...
    # Model
    in_channels: int = field(default=3, metadata={'help': 'Number of input channels.'})
    out_channels: int = field(default=1, metadata={'help': 'Number of output channels.'})
    features: int = field(default=64, metadata={'help': 'Number of features in first layer.'})
    # Training
    batch_size: int = field(default=8, metadata={'help': 'Batch size for training.'})
    learning_rate: float = field(default=1e-4, metadata={'help': 'Learning rate for training.'})
    weight_decay: float = field(default=1e-4, metadata={'help': 'Weight decay for training.'})
    adam_epsilon: float = field(default=1e-8, metadata={'help': 'Epsilon for Adam optimizer.'})
